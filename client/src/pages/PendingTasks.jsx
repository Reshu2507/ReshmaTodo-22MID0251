...
function PendingTasks() {
  ...
  const fetchPendingTodos = async () => {
    try {
      ...
      if (res.success) {
        setTodos(res.data.filter((t) => t.status === 'pending'));
      } else {
  ...
  const handleToggleStatus = async (todo) => {
    ...
    try {
      ...
      if (res.success) {
        // Since this page displays only pending, remove if toggled to completed
  ...
  const handleUpdateTodo = async (todoData) => {
    ...
    try {
      ...
      if (res.success) {
        // If status was changed from pending, filter it out
  ...
  return (
    <div className="todo-dashboard">
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <h2 style={{ fontSize: '2rem', fontWeight: 800, background: 'linear-gradient(to right, #f59e0b, #fbbf24)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          Pending Tasks
        </h2>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginTop: '0.25rem' }}>
          Total pending tasks: <strong style={{ color: 'var(--warning)' }}>{todos.length}</strong>
        </p>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Hooray! You don't have any pending tasks. Create a new task or sit back and relax!"
        />
      ) : (
